...
ILayer *addLSTM(INetworkDefinition *network, std::map<std::string, Weights>& weightMap, ITensor& input, int nIn, int nHidden, std::string lname) {
...
ICudaEngine* createEngine(unsigned int maxBatchSize, IBuilder* builder, IBuilderConfig* config, DataType dt) {
    ...
    // rnn
    auto lstm0 = addLSTM(network, weightMap, *sfl->getOutput(0), 512, 256, "rnn.0.rnn");
    ...
    auto lstm1 = addLSTM(network, weightMap, *sfl->getOutput(0), 256, 256, "rnn.1.rnn");
